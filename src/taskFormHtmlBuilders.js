...
function buildNewChecklistItemInputHtml() {
  const newChecklistItemInputHtml = buildInputHtml(
    "text",
...
function buildNewTaskTagSelectHtml() {
  ...
  const newTaskTagInputDefaultOption = buildSelectOption(
...
function buildTaskCompletedToggleBtnHtml() {
  const taskCompletedToggleBtnHtml = buildInputHtml(
    "button",
...
export function buildEmptyTaskFormHtml() {
  ...
  const labeledDueDateInputHtml = buildLabeledDateInputHtml(
    DUE_DATE_INPUT_LABEL_TEXT,
  );
  const titleDateHtml = wrapHtmlElements(
    wrapperType,
    titleInputHtml,
  ...
  const taskTagsListWithSelectHtml = wrapHtmlElements(
    wrapperType,
    taskTagsListHtml,
  ...
  const checklistWithInputHtml = wrapHtmlElements(
    wrapperType,
    checklistHtml,
  ...
  const taskFormButtons = wrapHtmlElements(
    "div",
    taskCompletedToggleBtnHtml,
    taskDeleteBtnHtml,
  );
  taskFormButtons.id = "task-form-btns";
  const formHtml = wrapHtmlElements(
    "form",
    titleDateHtml,
    shortDescHtml,
...
export function buildChecklistItemHtml(text, isChecked) {
  ...
  const newChecklistItemHtml = wrapHtmlElements(
    "li",
    itemCheckBoxHtml,
    itemTextInputHtml,
    itemDeleteIconHtml,
...
export function buildTaskTagListItemHtml(tagText) {
  ...
  const taskTagListItemHtml = wrapHtmlElements(
    "li",
    itemTextHtml,
    itemDeleteIconHtml,
  );
